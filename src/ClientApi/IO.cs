...
namespace ClientApi
{
    public class IO
    {
        #region ReceiveMessages and Events
        public  static IO Instance = new IO();
        public ClientSocket Connection = new ClientSocket();
        IO()
        {
            Connection.PropertyChanged += new System.ComponentModel.PropertyChangedEventHandler(p_PropertyChanged);
        }
        MyPacket msg;
        void p_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            Content = JsonFactory.GetObjPacket(Connection.Content);
        ...
        {
            String s = JsonFactory.GetJsonPacket(usList, null);
            Connection.SendMsg(s);
